...
#if psql -d gis -c '' 2>/dev/null; then 
...
#    # also for some reason the test above is failing?
#    echo ""
#    echo "changed=no"
#    exit 0
#fi
...
psql -d $template -f /usr/share/postgresql/{{ pillar.tm_postgresversion }}/contrib/postgis-{{ pillar.tm_postgisversion }}/postgis.sql > /dev/null
psql -d $template -f /usr/share/postgresql/{{ pillar.tm_postgresversion }}/contrib/postgis-{{ pillar.tm_postgisversion }}/spatial_ref_sys.sql > /dev/null
psql -d $template -f /usr/share/postgresql/{{ pillar.tm_postgresversion }}/contrib/postgis-{{ pillar.tm_postgisversion }}/postgis_comments.sql > /dev/null
